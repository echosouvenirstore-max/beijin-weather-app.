...
def create_excel_report(yearly_data, monthly_data, avg_rain, wet_month, insight):
    ...
    yearly_summary = pd.DataFrame({
        ...
        'Total Rainfall (mm)': yearly_data['Rainfall (mm)'],  # numeric
        'Status': yearly_data['Status']
    })
...
for year, rain in yearly_rain.items():
    ...
    if anom_val > (avg_rain * 0.15):
        status = "Wet"
    elif anom_val < -(avg_rain * 0.15):
        status = "Dry"
    else:
        status = "Normal"
    ...
    summary_data.append({
        "Year": year,
        "Rainfall (mm)": round(rain, 2),   # FIXED — numeric
        "Status": status
    })
...
if wet_months >= 5:
    insight_text = "Excellent Condition: Suitable for long-cycle crops like Rice."
    st.success(f"✅ Good: {insight_text}")

elif wet_months >= 3:
    insight_text = "Moderate Condition: Suitable for short-cycle crops such as Maize and Beans."
    st.warning(f"⚠️ Moderate: {insight_text}")

else:
    insight_text = "Dry: Recommended drought-resistant crops (Cassava, Sorghum)."
    st.error(f"🛑 Dry: {insight_text}")
...
ax1.set_title('Yearly Rainfall Anomaly')
ax1.set_ylabel('Deviation (mm)')
ax1.grid(axis='y', linestyle='--', alpha=0.5)

# Monthly Seasonality
ax2.plot(monthly_avg.index, monthly_avg, marker='o', color='green')
ax2.fill_between(monthly_avg.index, monthly_avg, color='green', alpha=0.2)
